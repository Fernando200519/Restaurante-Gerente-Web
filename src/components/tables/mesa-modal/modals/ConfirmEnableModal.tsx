...
import { CheckCircle, X, Loader2, Power } from "lucide-react";
...
export const ConfirmEnableModal: React.FC<Props> = ({
  ...
  return (
    <div className="fixed inset-0 z-70 flex items-center justify-center p-4">
      {/* 🌫️ Fondo con desenfoque premium */}
      <div
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 relative z-10 animate-in zoom-in-95 duration-300 border border-white/20">
        ...
        <div className="flex flex-col items-center text-center">
          {/* 🟢 Icono de Acción Positiva */}
          <div className="w-16 h-16 bg-emerald-50 rounded-3xl flex items-center justify-center mb-6 text-emerald-500 shadow-sm border border-emerald-100">
          ...
          <p className="text-gray-400 text-sm font-bold leading-relaxed mb-8 px-4">
            La unidad <span className="text-emerald-600">"{nombre}"</span>{" "}
            pasará a estado <span className="text-gray-900">Disponible</span>.
            Podrá recibir comandas de inmediato.
          </p>
          ...
          <div className="flex gap-4 w-full">
            ...
            <button
              onClick={onConfirm}
              disabled={loading}
              className={`${btnBase} bg-emerald-500 text-white shadow-lg shadow-emerald-200 hover:bg-emerald-600`}
            >
              {loading ? (
                <>
