...
const CategoryTable: React.FC<CategoryTableProps> = ({
  ...
  const organizedCategories = useMemo(() => {
    ...
    const sortLogic = (a: Category, b: Category) => {
      if (a.type === "Alimentos" && b.type === "Bebidas") return -1;
      if (a.type === "Bebidas" && b.type === "Alimentos") return 1;
    ...
    const flatten = (
      ...
      children.sort(sortLogic).forEach((child) => {
        ...
        flatten(
          child.id,
          depth + 1,
          isParentCollapsed || collapsedIds.has(child.id)
        );
      });
    };

    flatten(null, 0, false);

    categories.sort(sortLogic).forEach((c) => {
      if (!addedIds.has(c.id))
        result.push({ ...c, depth: 0, hasChildren: false });
    });

    return result.filter((cat) => String(cat.id) !== "3");
            ...
            return (
              ...
              >
                <td className="px-8 py-5 whitespace-nowrap">
                  ...
                  >
                    {/* 🆕 BOTÓN DE CONTROL DE JERARQUÍA */}
                    {category.hasChildren ? (
                      <button
                        onClick={() => toggleCollapse(category.id)}
                        className="p-1 hover:bg-orange-100 rounded-lg transition-colors cursor-pointer text-[#FF8108]"
                      >
                        {isCollapsed ? (
