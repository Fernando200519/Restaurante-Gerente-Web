...
const ProductModal: React.FC<ProductModalProps> = ({
  ...
  useEffect(() => {
    if (product && isOpen) {
      ...
      console.log(
        "ProductModal: Categorías activas:",
        categories.filter((cat) => cat.status === "activo")
      );
      setFormData({
        name: product.name,
  ...
  const onFileSelected = async (file?: File) => {
    ...
    if (!file.type.startsWith("image/")) {
      setErrors((prev) => ({
        ...prev,
        imageFile: "Solo se permiten imágenes",
      }));
      return;
    }
    ...
    try {
      ...
      if (file.size > MAX_SIZE_BYTES) {
        setErrors((prev) => ({
          ...prev,
          imageFile:
            "La imagen fue subida sin compresión y supera 5MB. Intente otra imagen.",
        }));
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {categories
                ...
                .map((category) => (
                  <option key={category.id} value={category.id}>
                    {category.name}{" "}
                    {category.status === "inactivo" ? "(Inactiva)" : ""}
                  </option>
                ))}
            </select>
            {categories.filter((cat) => cat.status === "activo").length ===
              0 && (
              <p className="mt-1 text-sm text-yellow-600">
                No hay categorías activas disponibles. Por favor, active una
                categoría primero.
              </p>
            )}
            {formData.categoryId &&
              ...
              !product && (
                <p className="mt-1 text-sm text-red-500">
                  No se pueden agregar productos a categorías inactivas. Por
                  favor, active la categoría primero.
                </p>
              )}
            {errors.categoryId && (
          ...
          {!formData.imageFile && !previewUrl ? (
            <div
              ...
              className={`w-full border-2 border-dashed rounded-lg p-6 text-center ${
                dragActive ? "border-primary bg-amber-50" : "border-gray-300"
              }`}
            >
        ...
        <div>
          ...
          {(() => {
            ...
            const isCategoryInactive =
              selectedCategory && selectedCategory.status === "inactivo";
            ...
            return (
              <>
                ...
                {isCategoryInactive && (
                  <p className="mt-1 text-sm text-yellow-600">
                    La categoría de este producto está inactiva. Los productos
                    de categorías inactivas no pueden estar activos. Por favor,
                    active la categoría primero si desea activar este producto.
                  </p>
                )}
              </>
            );
        ...
        <div className="flex justify-end gap-4 pt-4 border-t border-gray-200">
          ...
          <button
            ...
            onMouseEnter={(e) =>
              (e.currentTarget.style.backgroundColor = "#FA9623")
            }
            onMouseLeave={(e) =>
              (e.currentTarget.style.backgroundColor = "#FF8108")
            }
          >
