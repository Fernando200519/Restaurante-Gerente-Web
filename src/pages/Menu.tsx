...
const Menu: React.FC = () => {
  ...
  const [isDeleteCategoryModalOpen, setIsDeleteCategoryModalOpen] =
    useState(false);
  const [categoryToDeleteWithProducts, setCategoryToDeleteWithProducts] =
    useState<Category | null>(null);
  const [isActivateProductsConfirmOpen, setIsActivateProductsConfirmOpen] =
    useState(false);
  ...
  const [isConfirmingActivateProducts, setIsConfirmingActivateProducts] =
    useState(false);
    ...
    try {
      ...
      if (selectedCategory) {
        ...
        if (
          ...
          selectedCategory.status === "activo"
        ) {
          ...
          if (productsInCategory.length > 0) {
            console.log(
              `Actualizando ${productsInCategory.length} producto(s) a inactivo porque la categoría se puso inactiva`
            );
            const updatePromises = productsInCategory.map(async (product) => {
              try {
                ...
                console.error(
                  `Error al actualizar producto ${product.id} a inactivo:`,
                  err
                );
        ...
        if (
          ...
          selectedCategory.status === "inactivo"
        ) {
          // Buscar todos los productos inactivos de esta categoría
          const inactiveProductsInCategory = products.filter(
            (prod) =>
              prod.categoryId === selectedCategory.id &&
  ...
  const handleConfirmActivateProducts = async () => {
    ...
    try {
      setError(null);
      const { formData, updatedCategory, selectedCategory } =
        pendingCategoryUpdate;
      console.log("Iniciando activación de productos...");
      console.log(
        "selectedCategory desde pendingCategoryUpdate:",
        selectedCategory
      );
      ...
      const inactiveProductsInCategory = products.filter(
        (prod) =>
          prod.categoryId === selectedCategory.id && prod.status === "inactivo"
      );

      if (inactiveProductsInCategory.length > 0) {
        console.log(
          `Activando ${inactiveProductsInCategory.length} producto(s) porque la categoría se activó`
        );
        const updatePromises = inactiveProductsInCategory.map(
          async (product) => {
            try {
              ...
              console.error(
                `Error al actualizar producto ${product.id} a activo:`,
                err
              );
              // Continuar con los demás productos aunque uno falle
            }
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    try {
      ...
      if (products.length === 0 && !productsLoading) {
        console.log(
          "Cargando productos para verificar si la categoría tiene productos..."
        );
        await loadProducts();
      }
      ...
      console.log(
        `Categoría ${category.name} tiene ${productsInCategory.length} producto(s)`
      );
  ...
  const handleDeleteCategoryWithProducts = async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Error al eliminar la categoría y productos. Por favor, intenta de nuevo.";
      setError(errorMessage);
    }
  };
  ...
  const handleMoveProductsToCategory = async (targetCategoryId: string) => {
    ...
    try {
      ...
      if (productsInCategory.length > 0) {
        console.log(
          `Moviendo ${productsInCategory.length} producto(s) a la categoría ${targetCategoryId}`
        );
        const updatePromises = productsInCategory.map(async (product) => {
          try {
            const productFormData: ProductFormData = {
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Error al mover productos y eliminar categoría. Por favor, intenta de nuevo.";
      setError(errorMessage);
    }
  };

  // Opción 2b: Crear nueva categoría y mover productos
  const handleCreateCategoryAndMoveProducts = async (
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Error al crear categoría y mover productos. Por favor, intenta de nuevo.";
      setError(errorMessage);
    }
  };
  ...
  const handleDeleteCategoryWithoutCategory = async () => {
    ...
    try {
      ...
      if (productsInCategory.length > 0) {
        console.log(
          `Dejando ${productsInCategory.length} producto(s) sin categoría`
        );
        ...
        const updatePromises = productsInCategory.map(async (product) => {
          try {
            ...
            console.error(
              `Error al actualizar producto ${product.id} sin categoría:`,
              err
            );
            // Continuar con los demás productos aunque uno falle
          }
      ...
      console.error(
        "Error al eliminar categoría dejando productos sin categoría:",
        err
      );
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Error al eliminar categoría. Por favor, intenta de nuevo.";
      setError(errorMessage);
    }
  };

  // Opción 3A: Mover productos a categoría existente y eliminar
  const handleMoveProductsToExisting = async (targetCategoryId: string) => {
    if (!categoryToDeleteWithProducts) return;
    ...
    try {
      ...
      console.log(
        `Moviendo productos de categoría ${categoryId} a ${targetCategoryId} y eliminando`
      );
      await categoriesAPI.moveProductsAndDelete(categoryId, targetCategoryId);
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Error al mover productos. Por favor, intenta de nuevo.";
      setError(errorMessage);
    }
  };

  // Opción 3B: Crear nueva categoría, mover productos y eliminar original
  const handleMoveProductsToNew = async (newCategoryName: string) => {
    if (!categoryToDeleteWithProducts) return;
    ...
    try {
      ...
      console.log(
        `Migrando productos de categoría ${categoryId} a nueva categoría "${newCategoryName}"`
      );
      await categoriesAPI.migrateProductsToNewCategory(
        categoryId,
        newCategoryName
      );
      ...
      console.error("Error al migrar productos a nueva categoría:", err);
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Error al crear categoría y migrar productos. Por favor, intenta de nuevo.";
      setError(errorMessage);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Error al eliminar. Por favor, intenta de nuevo.";
      setError(errorMessage);
    }
  };
  ...
  const handleSaveProduct = async (formData: ProductFormData) => {
    try {
      ...
      const finalFormData: ProductFormData = {
        ...formData,
        status:
          category && category.status === "inactivo"
            ? "inactivo"
            : formData.status,
      };
            ...
            prod.id === selectedProduct.id
              ? {
                  ...
                  description:
                    ...
                    prod.description, // Fallback
                  categoryId:
                    updatedProduct.categoryId ||
                    formData.categoryId ||
                    prod.categoryId, // Fallback
                  categoryName:
                    category?.name ||
                    updatedProduct.categoryName ||
                    prod.categoryName, // Fallback
                  status:
                    finalFormData.status ||
                    updatedProduct.status ||
                    prod.status, // Usar finalFormData.status (puede estar forzado a inactivo si la categoría está inactiva)
                }
              : prod
          )
        );
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedCategoryFilter) {
      filtered = filtered.filter(
        (prod) => prod.categoryId === selectedCategoryFilter
      );
    }
    ...
    <div className="min-h-screen bg-gray-50">
                      ...
                      <>
                        ...
                        <div className="mt-6 flex items-center justify-center gap-4">
                          ...
                          <span className="text-sm text-gray-600">
                            Página {Math.min(currentPage, totalPages)} de{" "}
                            {totalPages}
                          </span>
      ...
      <ConfirmModal
        ...
        message={
          pendingCategoryUpdate && pendingCategoryUpdate.selectedCategory
            ? `¿Desea activar también todos los productos inactivos de la categoría "${
                pendingCategoryUpdate.selectedCategory.name
              }"? Esto activará ${
                products.filter(
                  (prod) =>
                    prod.categoryId ===
                      pendingCategoryUpdate.selectedCategory.id &&
                    prod.status === "inactivo"
                ).length
              } producto(s).`
      ...
      <ConfirmModal
        ...
        message={`¿Está seguro de que desea eliminar ${
          categoryToDelete ? "esta categoría" : "este producto"
      ...
      {/* Modal para eliminar categoría con opciones */}
      <DeleteCategoryModal
        ...
        productsCount={
          categoryToDeleteWithProducts
            ? products.filter(
                (prod) => prod.categoryId === categoryToDeleteWithProducts.id
              ).length
            : 0
        }
        categories={categories}
        ...
        onMoveProductsToExisting={handleMoveProductsToExisting}
        onMoveProductsToNew={handleMoveProductsToNew}
      />
    </div>
  );
};
