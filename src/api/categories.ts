...
const mapAPIResponseToCategory = (
  category: CategoryResponse,
  fallbackStatus?: Category["status"]
): Category => {
...
export const categoriesAPI = {
  ...
  getAll: async (): Promise<Category[]> => {
    try {
      ...
      console.log(
        "Respuesta completa de GET /categories:",
        JSON.stringify(response.data, null, 2)
      );
      if (response.data.length > 0) {
        console.log("Primera categoría recibida:", response.data[0]);
        console.log(
          "Campos de la primera categoría:",
  ...
  create: async (formData: CategoryFormData): Promise<Category> => {
    try {
      ...
      console.log(
        "Respuesta completa de la API al crear:",
        JSON.stringify(response.data, null, 2)
      );
      ...
      const mappedCategory = mapAPIResponseToCategory(
        response.data,
        formData.status
      );
      console.log(
        "Categoría mapeada (usando fallback si es necesario):",
        mappedCategory
      );
  ...
  update: async (id: string, data: CategoryFormData): Promise<Category> => {
    try {
      ...
      if (!response.data || Object.keys(response.data).length === 0) {
        console.log(
          "La respuesta está vacía, construyendo categoría desde los datos enviados"
        );
      ...
      if (
        response.data.descripcion === undefined ||
        response.data.descripcion === null
      ) {
  ...
  deleteWithProducts: async (id: string): Promise<void> => {
    try {
      ...
      console.log(
        "Intentando eliminar categoría con productos, ID:",
        categoryId
      );
      console.log("Endpoint:", endpoint);
      console.log("URL completa:", `${apiClient.defaults.baseURL}${endpoint}`);
      ...
      const response = await apiClient.delete(endpoint);
      console.log(
        "Respuesta de DELETE with-products exitosa:",
        response.status,
        response.data
      );
      return;
  ...
  // Mover productos a otra categoría existente y luego eliminar la categoría original
  // Opción A: POST /categories/{id}/move-products?targetCategoryID={targetId} + DELETE /categories/{id}
  moveProductsAndDelete: async (
    id: string,
    ...
    try {
      ...
      if (isNaN(targetId)) {
        throw new Error(
          `ID de categoría destino inválido: ${targetCategoryId}`
        );
      }

      // Paso 1: Mover productos a la categoría destino
      const moveEndpoint = `/categories/${categoryId}/move-products?targetCategoryID=${targetId}`;
      console.log(
        "Moviendo productos de categoría",
        categoryId,
        "a categoría",
        targetId
      );
      console.log("Endpoint move-products:", moveEndpoint);
      await apiClient.post(moveEndpoint);
      console.log("Productos movidos exitosamente");

      // Paso 2: Eliminar la categoría original (ahora vacía)
      const deleteEndpoint = `/categories/${categoryId}`;
      console.log("Eliminando categoría original:", categoryId);
      await apiClient.delete(deleteEndpoint);
      console.log("Categoría eliminada exitosamente");

      return;
    } catch (error: any) {
      console.error("Error al mover productos y eliminar categoría:", error);
      if (error.response) {
        console.error("Response data:", error.response.data);
        console.error("Response status:", error.response.status);
        console.error("Request URL:", error.config?.url);
  ...
  // Migrar productos a una nueva categoría (crearla y mover) y eliminar la original
  // Opción B: POST /categories/{id}/migrate-products?newCategoryName={name}
  migrateProductsToNewCategory: async (
    id: string,
    ...
    try {
      ...
      const endpoint = `/categories/${categoryId}/migrate-products?newCategoryName=${encodeURIComponent(
        newCategoryName.trim()
      )}`;
      console.log("Migrando productos a nueva categoría:", newCategoryName);
      console.log("Endpoint migrate-products:", endpoint);

      await apiClient.post(endpoint);
      console.log(
        "Productos migrados y categoría original eliminada exitosamente"
      );

      return;
    } catch (error: any) {
      console.error("Error al migrar productos a nueva categoría:", error);
      if (error.response) {
        console.error("Response data:", error.response.data);
        console.error("Response status:", error.response.status);
        console.error("Request URL:", error.config?.url);
