...
const mapEstadoIdToStatus = (
  ...
  if (stateNombre) {
    ...
    if (nombreLower.includes("activo") && !nombreLower.includes("inactivo"))
      return "activo";
...
const buildMultipartPayload = (
  ...
  if (formData.imageFile) {
    fd.append("FormFile", formData.imageFile); // FormFile con F mayúscula según Swagger
  }
...
export const productsAPI = {
  ...
  getAll: async (
    ...
    try {
      ...
      console.log(
        "Respuesta completa de GET /products:",
        JSON.stringify(response.data, null, 2)
      );
      return response.data.map((product) =>
        mapAPIResponseToProduct(product, categories)
      );
    } catch (error) {
  ...
  getFormData: async (): Promise<ProductFormDataResponse> => {
    try {
      const response = await apiClient.get<ProductFormDataResponse>(
        "/products/form-data"
      );
  ...
  create: async (
    ...
    try {
      ...
      const mappedProduct = mapAPIResponseToProduct(
        response.data,
        categories,
        formData.price,
        formData.status
      ...
      console.log(
        "Producto mapeado (usando fallback si es necesario):",
        mappedProduct
      );
  ...
  update: async (
    ...
    try {
      ...
      // Lógica de imagen según el backend:
      // CambiarImagen (required boolean):
      //   - true + FormFile: reemplazar imagen
      //   - true + sin FormFile: eliminar imagen
      //   - false: no tocar la imagen
      console.log("[products.update] Estado de imagen:", {
        tieneImageFile: !!formData.imageFile,
        removeImage: formData.removeImage,
      });
      if (formData.imageFile) {
        // Hay nueva imagen: cambiar y adjuntar archivo
        fd.append("CambiarImagen", "true");
        fd.append("FormFile", formData.imageFile); // FormFile con F mayúscula según Swagger
        console.log(
          "[products.update] Enviando CambiarImagen=true con FormFile"
        );
      } else if (formData.removeImage) {
        // Usuario quitó la imagen: cambiar pero sin archivo (eliminar)
        fd.append("CambiarImagen", "true");
        console.log(
          "[products.update] Enviando CambiarImagen=true SIN formFile (eliminar)"
        );
      } else {
        // No hay cambios en la imagen
        fd.append("CambiarImagen", "false");
        console.log(
          "[products.update] Enviando CambiarImagen=false (sin cambios)"
        );
      }
      ...
      console.log(
        "Respuesta completa de la API al actualizar:",
        JSON.stringify(response.data, null, 2)
      );
      console.log("Precio en respuesta:", response.data.precio);
      ...
      if (!response.data || Object.keys(response.data).length === 0) {
        console.log(
          "La respuesta está vacía, construyendo producto desde los datos enviados"
        );
        ...
        return mapAPIResponseToProduct(
          fallbackProduct,
          categories,
          formData.price,
          formData.status
  ...
  delete: async (id: string): Promise<void> => {
    try {
      ...
      console.log(
        "Intentando eliminar producto con ID:",
        productId,
        "(tipo: number)"
      );
